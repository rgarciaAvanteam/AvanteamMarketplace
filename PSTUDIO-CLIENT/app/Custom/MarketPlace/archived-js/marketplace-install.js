 ...
 * @param {string} level - Niveau de log (INFO, WARNING, ERROR, SUCCESS, SCRIPT, SCRIPT_SECTION)
 */
...
function installComponent(componentId, version) {
    ...
    // Utiliser le streaming de logs si disponible
    const useStreaming = typeof marketplaceStream !== 'undefined';
    
    if (useStreaming) {
        // Initialiser le stream
        marketplaceStream.init({
            installId: installId,
    ...
    fetch(`${apiUrl}/components/${componentId}/download?clientId=${encodeURIComponent(clientId)}&version=${encodeURIComponent(version)}&urlOnly=true&installId=${encodeURIComponent(installId)}`, {
        method: 'POST',
        headers: headers
    })
    .then(response => {
    ...
    .then(data => {
        ...
        return installComponentPackage(data.downloadUrl, componentId, version, progressBar, progressText, logContainer, installId, useStreaming);
    })
    .then((installResult) => {
 ...
 * @param {HTMLElement} progressText - Élément de texte de progression
 * @param {HTMLElement} logContainer - Conteneur de log
 * @param {string} installId - ID d'installation unique
 * @param {boolean} useStreaming - Si le streaming de logs est utilisé
 * @returns {Promise} Promise résolue lorsque l'installation est terminée
 */
function installComponentPackage(downloadUrl, componentId, version, progressBar, progressText, logContainer, installId, useStreaming) {
    return new Promise((resolve, reject) => {
        ...
        .then(result => {
            ...
            // DIAGNOSTIC: Afficher les détails des logs pour déboguer
            console.log("Nombre de logs:", result.logs ? result.logs.length : 0);
            if (result.logs && Array.isArray(result.logs)) {
                console.log("Premier log:", result.logs[0]);
                console.log("Dernier log:", result.logs[result.logs.length - 1]);
                console.log("Types de niveaux:", [...new Set(result.logs.map(log => log.level))]);
                console.log("Nombre de logs de type SCRIPT:", result.logs.filter(log => log.level === "SCRIPT").length);
            }
