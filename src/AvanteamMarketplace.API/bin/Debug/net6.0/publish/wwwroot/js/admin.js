...
function loadComponentVersions(componentId) {
    ...
    .then(data => {
        ...
        versions.forEach(version => {
            ...
            row.innerHTML = `
                ...
                <td>${version.minPlatformVersion || 'N/A'}</td>
                <td>${version.downloadCount}</td>
                <td>${version.isLatest ? '<span class="is-latest-badge">Actuelle</span>' : ''}</td>
                <td class="action-buttons">
                    <a href="#" class="action-btn action-btn-edit-version" data-id="${version.versionId}">Modifier</a>
                    <a href="#" class="action-btn action-btn-set-latest" data-id="${version.versionId}" ${version.isLatest ? 'style="display:none"' : ''}>Définir comme actuelle</a>
                </td>
            `;
    ...
    .catch(error => {
        ...
        alert('Erreur lors du chargement des versions. Consultez la console pour plus de détails.');
    });
}
...
function showVersionModal(versionId = null) {
    ...
    if (versionId) {
        ...
        .then(response => response.json())
        .then(data => {
            $('#txtVersionNumber').val(data.versionNumber);
            $('#txtVersionChangelog').val(data.changeLog);
            $('#txtVersionMinPlatform').val(data.minPlatformVersion || '');
            $('#chkIsLatestVersion').prop('checked', data.isLatest);
...
function saveVersion() {
    ...
    if (currentVersionId) {
        ...
        // Create new version
        if (!hasFile && !versionData.PackageUrl) {
            alert('Veuillez sélectionner un fichier de package ou fournir une URL de package');
            return;
        }
        
        if (hasFile) {
            // Upload file with version data
            const formData = new FormData();
            formData.append('packageFile', fileInput.files[0]);
            
            fetch(`${apiBaseUrl}/management/components/${currentComponentId}/versions`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${adminToken}`
                },
            ...
            .catch(error => {
                console.error('Erreur:', error);
                alert('Erreur lors de la création de la version: ' + error.message);
            });
        } else {
            // Create version with just the data
            fetch(`${apiBaseUrl}/management/components/${currentComponentId}/versions`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
...
$(document).ready(function() {
    ...
    $("#fileVersionPackage").change(function() {
        const fileName = $(this).val().split('\\').pop();
        $("#selectedVersionFileName").text(fileName);
    });
    ...
    $("#btnUploadPackage").click(function() {
        ...
        const originalButtonText = $("#btnUploadPackage").text();
        $("#btnUploadPackage").html('<i class="fas fa-spinner fa-spin"></i> Téléversement...');
        $("#btnUploadPackage").prop('disabled', true);
        ...
        $.ajax({
            ...
            error: function(xhr, status, error) {
                ...
                $("#btnUploadPackage").html(originalButtonText);
                $("#btnUploadPackage").prop('disabled', false);
            }
        });
    });
    ...
    $("#btnUploadIcon").click(function() {
        ...
        // Afficher un indicateur de chargement
        const originalButtonText = $("#btnUploadIcon").text();
        $("#btnUploadIcon").html('<i class="fas fa-spinner fa-spin"></i> Téléversement...');
        $("#btnUploadIcon").prop('disabled', true);
        ...
        $.ajax({
            ...
            error: function(xhr, status, error) {
                ...
                // Rétablir le bouton à son état d'origine
                $("#btnUploadIcon").html(originalButtonText);
                $("#btnUploadIcon").prop('disabled', false);
            }
        });
    });
